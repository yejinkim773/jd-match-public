...
def extract_text_from_image(image_bytes: bytes) -> str:
    return _call([
        {"text": (
            ...
            "소제목, 섹션 헤더, 날짜, 수치 등 모든 텍스트를 원문 그대로 추출하세요. "
            "이미지에 있는 언어를 그대로 유지하세요 (영어는 영어로, 한국어는 한국어로). "
            "한자(중국어)나 일본어로 변환하지 마세요. "
...
def extract_texts_from_images(images_bytes: list[bytes]) -> str:
    ...
    parts: list = [{"text": (
        ...
        "절대 요약하거나 생략하지 마세요. 소제목, 섹션 헤더, 날짜, 수치 등 모든 텍스트를 원문 그대로 추출하세요. "
        "페이지 사이는 '---' 한 줄로 구분해주세요. "
        "이미지에 있는 언어를 그대로 유지하세요 (영어는 영어로, 한국어는 한국어로). "
        "한자(중국어)나 일본어로 변환하지 마세요. "
        "원문의 줄바꿈 구조를 최대한 유지해주세요."
